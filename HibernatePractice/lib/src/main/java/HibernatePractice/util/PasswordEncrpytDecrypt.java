...
public class PasswordEncrpytDecrypt {

	public static SecretKey getKeyFromPassword(String password)
			throws NoSuchAlgorithmException, InvalidKeySpecException {

		SecretKeyFactory factory = SecretKeyFactory.getInstance(Constants.SECUREKEYINS);
		KeySpec spec = new PBEKeySpec(password.toCharArray(), Constants.ENCSALT.getBytes(), 65536, 256);
		SecretKey secret = new SecretKeySpec(factory.generateSecret(spec).getEncoded(), Constants.ENCALGO);
		return secret;
	}
	...
	public static IvParameterSpec generateIv(byte[] ivByte) {
